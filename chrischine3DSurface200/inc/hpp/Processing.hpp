...
*/
	...
	**/
	...
	public:
		...
		*/
		void generatePoints(unsigned short *depthMap, int width, int height, float depthScale,Intrinsics *intrinsicsDepth, Intrinsics *intrinsicsRgb, Extrinsics *extrinsics, bool removeInvalid = false)
		{
			_points.clear();
			_textures.clear();
			_normals.clear();
		...
		*								it should be setted to nullptr if no texture
		**/
		void generatePoint(float3& vertex, float2& textureCoord, int u, int v, unsigned short depth, int width, int height, float depthScale,Intrinsics *intrinsicsDepth, Intrinsics *intrinsicsRgb = nullptr, Extrinsics *extrinsics = nullptr)
		{
			Intrinsics intrDepth;
			memcpy(&intrDepth, intrinsicsDepth, sizeof(Intrinsics));
